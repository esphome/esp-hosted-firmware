 ...
 *   - the host shim   (components/esp_now_hosted/esp_now_hosted.cpp)
 *   - the C6 slave ext (c6-slave-espnow/esp_now_hosted_slave.c)
 *
 ...
 *
 * KEEP THE TWO COPIES IN SYNC. The canonical copy lives here; the slave build
 * uses a verbatim copy. Both sides are little-endian, so these packed structs
 * are wire-compatible with no byte-swapping.
 */
 ...
 * One REQ handler slot on the slave; three event handler slots on the host.
...
#define ESP_NOW_HOSTED_MSG_REQ   0x6E6F7701u /* host  -> slave : request envelope */
#define ESP_NOW_HOSTED_MSG_RESP  0x6E6F7702u /* slave -> host  : reply to a REQ   */
#define ESP_NOW_HOSTED_MSG_RECV  0x6E6F7703u /* slave -> host  : async RX frame   */
#define ESP_NOW_HOSTED_MSG_SEND  0x6E6F7704u /* slave -> host  : async TX status  */

/* ── Request opcodes ────────────────────────────────────────────────────── */
enum {
  ESP_NOW_HOSTED_OP_INIT          = 1, /* esp_now_init + register slave recv/send cbs */
